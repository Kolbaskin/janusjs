...
Server.prototype.respondNoGzip = function (pathname, status, contentType, _headers, files, stat, req, res, finish) {
    ...
    me.prepareFile(files, function(data) {
        if(data) stat.size = Buffer.byteLength(data, 'utf8');
        me.respondNoGzip1(pathname, status, contentType, _headers, files, stat, req, res, finish, data);       
    });
}
Server.prototype.respondNoGzip1 = function (pathname, status, contentType, _headers, files, stat, req, res, finish, data) {    
...
Server.prototype.prepareFile = function(files, cb) {
    if(files.length == 1 && /\/model\/[a-z0-9]{1,}\.js$/i.test(files[0])) {
        fs.readFile(files[0], 'utf-8', function(e, s) {
            var c, j, l, i = 0, out = '';
