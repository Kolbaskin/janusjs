Ext.define('Core.plugins.GridReorderDnD', {
    ...
    onGridRender: function(grid){
        ...
        this.target = new Ext.dd.DropTarget(grid.getEl(), {
            ...
            notifyDrop: function(dd, e, data){
                ...
                if (!self.copy) {
                    for (var i = 0; i < data.selections.length; i++) {
                        ds.addListener('beforewrite', function(ds, action, rs, o){
                            if (action == 'destroy') {
                                if (Ext.isArray(rs)) {
                                    Ext.each(rs, function(){
                                        for (var k = 0; k < Ext.ux.deleted.length; k++) {
                                            if (this.get('id') == Ext.ux.deleted[k]) {
                                                Ext.ss.ScanTokens.destroy(this.get('id'), function(){
                                    ...
                                    for (var k = 0; k < Ext.ux.deleted.length; k++) {
                                        if (rs.get('id') == Ext.ux.deleted[k]) {
                                            Ext.ss.ScanTokens.destroy(rs.get('id'), function(){
                ...
                var j = 0;
                ds.each(function(){
                    if (this.get('index') >= parseInt(data.selections[0].get('index'))) {
                        this.set('index', j);
                    }
                    j++;
                });
                ...
                self.fireEvent(self.copy ? 'afterrowcopy' : 'afterrowmove', this.gridDropTarget, data.rowIndex, rindex, data.selections);
                return true;
            },
